//! 翻译结果与结构化词条缓存。
//!
//! 翻译器缓存译文，词典模块缓存序列化后的 AI 词条；两者共用同一份
//! JSON 文件和原子写入机制，并通过不同缓存键保持隔离。
//!
//! - **省请求与额度**：重复翻译或查词不必再次访问网络模型。
//! - **单个 JSON 文件**：个人使用量级下读写开销可忽略，也无需新增依赖。
//! - **SHA-256 键**：避免哈希碰撞导致返回错误的缓存内容。
//! - **损坏可再生**：解析失败就当空缓存，不让缓存故障阻断工具使用。
...
/// 缓存句柄。
///
/// 生命周期：`load()` 读入内存 → 调用方 `get` / `insert` → 结束时 `save()`。
/// 不在每次读写时碰磁盘，避免频繁 IO。
pub struct Cache {
    /// 缓存文件路径
    path: PathBuf,
...
impl Cache {
    ...
    /// 查询缓存。未命中返回 None。
    ///
    /// 不在这里统计命中率；翻译调度器和词典查询分别记录自己的结果来源。
...
/// 生成翻译或词条结果的缓存键。
///
/// 键由所有影响结果的因素构成：后端名、模型或格式版本、源语种、目标语种、
/// 备用目标语和原始输入。任一项不同都会得到不同的键，避免串用结果。
///
/// `to_alt` 是「输入语种恰好等于目标语种」时改用的备用目标语。
pub fn cache_key(
    backend: &str,
    backend_detail: &str,
    from: &str,
